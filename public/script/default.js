...
function trackLargestResource() {
  const observer = new PerformanceObserver((list) => {
    ...
    if (entries.length) {
      ...
      console.log(
        `Largest resource: ${slowest.name} (${slowest.size / 1000}KB)`
      );
    }
  });
  ...
  const getLastCommitDate = async () => {
    try {
      const response = await fetch(
        "https://api.github.com/users/Tristan-Phillips/events"
      );
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const LIVE_PROJECTS = [
    {
      name: "Paintedd.Store",
      description:
        "Digital capitalism simulator powered by Shopify and artistic desperation",
    ...
    },
    {
      ...
      description:
        "Automotive prayer wheel for when your car breaks down again",
        ...
        card.innerHTML = `
                    ...
                    <a href="${
                      project.url
                        ...
                        Visit ${
                          project.status === "beta" ? "Beta" : "Live Site"
                        }
                    </a>
                `;
  ...
  if (avatar) {
    document.addEventListener("mousemove", (e) => {
      ...
      const rotateX = Math.min(Math.max(-(y - centerY) / 10, -15), 15).toFixed(
        2
      );
      const rotateY = Math.min(Math.max((x - centerX) / 10, -15), 15).toFixed(
        2
      );
